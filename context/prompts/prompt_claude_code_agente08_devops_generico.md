...
Sua missão é construir **somente o agente `Agente08_DevOps`**, usando os arquivos genéricos da fábrica como fonte de build.

Este processo é **build-time only**.

O agente gerado NÃO deve depender de documentos globais em runtime.
Depois do build, o `Agente08_DevOps` deve operar apenas com os arquivos locais gerados dentro da própria pasta:
...
Este agente será o **DevOps / Deployment / SRE** da fábrica.

Ele será responsável por:

- configurar deploy Vercel-first
- criar CI/CD
- validar env vars
- planejar migrations
...
- definir healthcheck
- criar rollback plan
- definir smoke tests e observabilidade
- emitir Post_Deploy_Report.md
...
Durante o runtime, o `Agente08_DevOps` NÃO deve depender desses arquivos globais.
...
```txt
O Agente08_DevOps deve ser autocontido após o build.
```
...
Se `Agente08_DevOps/` não existir, crie.

Gere ou atualize:

```txt
build/Agente08_DevOps_scan_report.md
build/missing_structure_report.md
```
...
- existência ou criação da pasta `Agente08_DevOps/`;
- fontes bibliográficas disponíveis para `DevOps / Deployment / SRE`;
...
Extraia somente o que for necessário para construir o `Agente08_DevOps`.
...
```txt
build/Agente08_DevOps_context_routing_plan.md
```
...
Leia a parte da bibliografia relacionada a `DevOps / Deployment / SRE` em:
...
```txt
build/Agente08_DevOps_bibliography_inventory.json
```
...
Agente08_DevOps/
  ...
  schemas/
    ...
    migration_deploy.schema.json
    incident_runbook.schema.json
  ...
  examples/
    ...
    good_post_deploy_report.md
    bad_post_deploy_report.md
```
...
```md
# Agente08_DevOps — DevOps / Deployment / SRE
...
O agente só pode consultar os artefatos locais dentro da pasta `Agente08_DevOps/` e os artefatos de projeto fornecidos como input pelo Tech Lead ou orquestrador.
...
```json
{
  ...
  "name": "DevOps / Deployment / SRE",
  "version": "1.0.0",
  "mode": "runtime-local-only",
  "edition": "generic-white-label",
  "primary_responsibility": "devops_deployment_sre",
...
Ele deve conter apenas o subconjunto necessário para o papel de **DevOps / Deployment / SRE**.
...
```json
{
  ...
  "collections": [
    ...
    {
      "name": "database_reliability",
      "priority": "supporting",
      "usage": "theoretical"
    },
    {
      "name": "architecture_reference_devops_view",
      "priority": "supporting",
      "usage": "normative"
    }
  ],
...
Inclua falhas específicas do papel de **DevOps / Deployment / SRE**.
...
Para cada skill dentro de `Agente08_DevOps/skills/`, crie:
...
- tratar Docker/Kubernetes como padrão quando Golden Path é Vercel
- gerar Dockerfile/manifests sem ADR ou requisito explícito
...
```txt
build/Agente08_DevOps_build_report.md
build/Agente08_DevOps_generated_files_index.md
build/Agente08_DevOps_runtime_readiness_checklist.md
```
...
Crie somente o `Agente08_DevOps`.
