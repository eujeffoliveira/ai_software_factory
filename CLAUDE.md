...
- `~/.codex/config.toml` — global Codex `mcp_servers.knowledge` entry
...
**What `install.ps1` does (8 phases):**
...
8. Merges `mcp_servers.knowledge` into `~/.codex/config.toml`, creates local `.codex/config.toml`, and generates helper scripts: `factory.ps1`, `update-knowledge.ps1`, `link-mcp.ps1`
...
- `link-mcp.ps1` is generated by the installer and lives at the factory root — it is machine-specific.
